...
const mg = mailgun({
  apiKey: "9bd20544d943a291e8833abd9e0c9908-76f111c4-8a189b96",
  domain: DOMAIN,
});
...
exports.forgetPassword = async (req, res, next) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "Password have been reset and sent to email",
    });
  } catch (error) {
    ...
    if (req.file) {
      ...
      cloudinary.v2.uploader.upload(
        ...
        async function (error, result) {
          ...
          const editedStaff = await Staff.findOne({
            username: req.query.username,
          });
...
exports.removeStaff = async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: `staff with the id ${username} has been removed`,
  });
};
...
exports.editStaff = async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: `staff with the username ${username} has been edited`,
  });
};
...
exports.addHomeEvent = async (req, res, next) => {
  ...
  if (homeEventType == "vc") {
    result = await HomePage.findOneAndUpdate(
      {},
      { $set: { [homeEventType]: evnt } },
      { new: true }
    );
  } else {
    result = await HomePage.findOneAndUpdate(
      {},
      { $push: { [homeEventType]: evnt } },
      { new: true }
    );
  }
  ...
  res.json({
    success: true,
    message: "Event created successfullty",
    result,
    newlyEvent: evnt,
  });
};
  ...
  singleAllMediaUpload(req, res, async function (err) {
    ...
    if (req.file) {
      ...
      if (req.file.mimetype == "application/pdf") {
        ...
        res.json({
          success: false,
          message: "please only upload pdf document",
        });
      }
    }
  });
...
exports.getAllDownloadsWithPagination = async (req, res, next) => {
  ...
  let result;
  try {
    const allDownloads = await Downloadable.find(
      {},
...
exports.getAllPortalsWithPagination = async (req, res, next) => {
  ...
  let result;
  try {
    const allPortals = await PortalLink.find(
      {},
      { _id: 0, createdAt: 0, updatedAt: 0, __v: 0 }
    );
    if (limit === "All") {
...
exports.getStatistics = async (req, res, next) => {
  try {
    ...
    const departmentList = await {
      NumberOfCount:
        (facultyDepartmentList.length > 0
          ? facultyDepartmentList[0]["NumberOfCount"]
          : (facultyDepartmentList = 0)) +
        (collegeDepartmentList.length > 0
          ? collegeDepartmentList[0]["NumberOfCount"]
          : (collegeDepartmentList = 0)) +
        (schoolDepartmentList.length > 0
          ? schoolDepartmentList[0]["NumberOfCount"]
          : (schoolDepartmentList = 0)),
      faculty:
        facultyDepartmentList.length > 0
          ? facultyDepartmentList[0]["NumberOfCount"]
          : (facultyDepartmentList = 0),
    ...
    const programList = await {
      NumberOfCount:
        (facultyProgramList.length > 0
          ? facultyProgramList[0]["NumberOfCount"]
          : (facultyProgramList = 0)) +
        (collegeProgramList.length > 0
          ? collegeProgramList[0]["NumberOfCount"]
          : (collegeProgramList = 0)) +
        (schoolProgramList.length > 0
          ? schoolProgramList[0]["NumberOfCount"]
          : (schoolProgramList = 0)) +
        (centerProgramList.length > 0
          ? centerProgramList[0]["NumberOfCount"]
          : (centerProgramList = 0)) +
        (unitProgramList.length > 0
          ? unitProgramList[0]["NumberOfCount"]
          : (unitProgramList = 0)),
    ...
    res.json({
      ...
      result: [
        ...result,
        { name: "programmes", count: programList["NumberOfCount"] },
      ],
    });
  } catch (error) {
    console.log(error);
...
exports.removeEvent = async (req, res, next) => {
  ...
  await HomePage.findOneAndUpdate(
    { evntId: eventId },
    { $pull: { [eventName]: { evntId: eventId } } }
  );
  res.json({
    success: true,
    message: `Event with the id ${eventId} has been removed`,
  });
};
...
exports.addFaculty = async (req, res, next) => {
  ...
  if (status === "faculty") entity.facultyId = randomstring.generate(20);
  else if (status === "college") entity.collegeId = randomstring.generate(20);
  else if (status === "school") entity.schoolId = randomstring.generate(20);
  else if (status === "center") entity.centerId = randomstring.generate(20);
  else if (status === "unit") entity.unitId = randomstring.generate(20);
  ...
  try {
    ...
    if (status === "faculty")
      inserter(Faculty, "Faculty created successfullty");
    else if (status === "college")
      inserter(College, "College created successfully");
    else if (status === "school")
      inserter(School, "School created successfully");
    else if (status === "center")
      inserter(Center, "Center created successfully");
    else if (status === "unit") inserter(Unit, "Center created successfully");
    else {
...
exports.createFacultyFromAfile = async (req, res, next) => {
  ...
  singleFileUpload(req, res, async function (err) {
    ...
    if (req.file) {
      ...
      if (activity === "faculty")
        await inserterFunction(Faculty, req.file.path);
      else if (activity === "college")
        await inserterFunction(College, req.file.path);
      else if (activity === "school")
        await inserterFunction(School, req.file.path);
      else if (activity === "center")
        await inserterFunction(Center, req.file.path);
...
// functiion
const getAllFacultiesOrSchoolOrCollege = async (
  Document,
  entityName,
  entityId
) => {
  try {
    return await Document.aggregate([
      ...
      {
        $project: {
          detail: { name: `$${[entityName]}`, id: `$${[entityId]}` },
          _id: 0,
        },
      },
...
exports.getAllFacultiesSchoolsCollege = async (req, res, next) => {
  try {
    const faculty = await getAllFacultiesOrSchoolOrCollege(
      Faculty,
      "facultyName",
      "facultyId"
    );
    const school = await getAllFacultiesOrSchoolOrCollege(
      School,
      "schoolName",
      "schoolId"
    );
    const college = await getAllFacultiesOrSchoolOrCollege(
      College,
      "collegeName",
      "collegeId"
    );
    const center = await getAllFacultiesOrSchoolOrCollege(
      Center,
      "centerName",
      "centerId"
    );
    const unit = await getAllFacultiesOrSchoolOrCollege(
      Unit,
      "unitName",
      "unitId"
    );
...
exports.getAllFaculties = async (req, res, next) => {
  if (req.params.activity === "faculty")
    result = await Faculty.find({}, { facultyName: 1, facultyId: 1 });
  else if (req.params.activity === "college")
    result = await College.find({}, { collegeName: 1, collegeId: 1 });
  else if (req.params.activity === "school")
    result = await School.find({}, { schoolName: 1, schoolId: 1 });
  else if (req.params.activity === "center")
    result = await Center.find({}, { centerName: 1, centerId: 1 });
  else if (req.params.activity === "unit")
    result = await Unit.find({}, { unitName: 1, unitId: 1 });
  else {
...
exports.singleFaculty = async (req, res, next) => {
  ...
  try {
    ...
    if (activity === "faculty")
      result = await Faculty.findOne({ [target]: eventId });
    else if (activity === "college")
      result = await College.findOne({ [target]: eventId });
    else if (activity === "school")
      result = await School.findOne({ [target]: eventId });
    else if (activity === "center")
      result = await Center.findOne({ [target]: eventId });
    else if (activity === "unit")
      result = await Unit.findOne({ [target]: eventId });
    else {
      res.json({ success: false, message: "Wrong parameters" });
    }
...
exports.getAllDepartment = async (req, res, next) => {
  ...
  try {
    const facultyDepartment = await allDepartment(
      Faculty,
      "facultyId",
      "facultyName",
      "faculty"
    );
    const collegeDepartment = await allDepartment(
      College,
      "collegeId",
      "collegeName",
      "college"
    );
    const schoolDepartment = await allDepartment(
      School,
      "schoolId",
      "schoolName",
      "school"
...
exports.editFaculty = async (req, res, next) => {
  const { newData, entityId, activity, target } = req.body;
  const editEntity = async (Document) => {
    const result = await Document.findOne({ [target]: entityId });
    if (!result) {
      res.status(404).json({ success: false, message: "Not found" });
      return;
    }

    await Document.findOneAndUpdate({ [target]: entityId }, newData, {
      new: true,
    });
  };

  try {
    if (activity == "faculty") await editEntity(Faculty);
    else if (activity == "college") await editEntity(College);
    else if (activity == "school") await editEntity(School);
    else if (activity == "center") await editEntity(Center);
    else if (activity == "unit") await editEntity(Unit);
    else {
    ...
    res.json({
      success: true,
      message: `Faculty with the ID ${entityId} has been edited`,
    });
  } catch (error) {
    console.log(error);
  }
...
exports.removeFaculty = async (req, res, next) => {
  ...
  try {
    const doDelete = async (Document) => {
      ...
      activity == "center" || activity == "unit"
        ? await Document.find({}, { dean: 0, programs: 0, staffList: 0 })
        : await Document.find({}, { dean: 0, departmentList: 0 });
      res.json({
        success: true,
        message: `Faculty with the ID ${entityId} has been removed`,
      });
    };
...
exports.addDean = async (req, res, next) => {
  const { dean, activity, target, entityId } = req.body;
  ...
  const createDean = async (Document) => {
    ...
    res.json({ success: true, message: "Dean created successfullty" });
    return;
  };

  try {
    if (activity == "faculty") await createDean(Faculty);
    else if (activity == "college") await createDean(College);
    else if (activity == "school") await createDean(School);
    else if (activity == "center") await createDean(Center);
    else if (activity == "unit") await createDean(Unit);
    else {
      res.status(400).json({ success: false, message: "wrong parameters" });
    }
  } catch (error) {
...
exports.removeDean = async (req, res, next) => {
  const { entityId, activity, target } = req.body;
  ...
  const deanRemove = async (Document) => {
    ...
    res.json({
      success: true,
      message: `Dean from faculty with the ID ${entityId} has been removed`,
    });
    return;
  };

  try {
    if (activity == "faculty") await deanRemove(Faculty);
    else if (activity == "college") await deanRemove(College);
    else if (activity == "school") await deanRemove(School);
    else if (activity == "center") await deanRemove(Center);
    else if (activity == "unit") await deanRemove(Unit);
    else {
      res.status(400).json({ success: false, message: "Wrong parameters" });
    }
  } catch (error) {
...
exports.editDepartment = async (req, res, next) => {
  ...
  try {
    if (activity == "faculty") result = await editSingleDepartment(Faculty);
      ...
      result = await editSingleDepartment(College);
  ...
  res.json({
    success: true,
    message: `Department from ${activity} with the ID ${departmentId} has been edited`,
    result,
  });
};
...
exports.addDepartmentStaff = async (req, res, next) => {
  ...
  const createAcademicStaff = async (Document) => {
    if (activity == "center" || activity == "unit") {
      ...
      await Document.findOneAndUpdate(
        { [target]: entityId },
        { $push: { staffList: staff } },
        { new: true }
      );
      return await Document.findOne(
        { [target]: entityId },
        { _id: 0, "staffList.password": 0 }
      );
    } else {
      await Document.findOneAndUpdate(
        { "departmentList.departmentId": entityId },
        { $push: { "departmentList.$.staffList": staff } },
        { new: true }
      );
      let result = await Document.findOne(
        { "departmentList.departmentId": entityId },
        { "departmentList.staffList.password": 0 }
      );
  ...
  try {
    if (activity == "faculty") {
      ...
      res.json({
        success: true,
        message: "Staff created successfullty",
        result,
      });
    } else if (activity == "college") {
...
exports.addDepartmentProgram = async (req, res, next) => {
  ...
  const addProgam = async (Document) => {
    if (activity == "center" || activity == "unit") {
      ...
      await Document.findOneAndUpdate(
        { "departmentList.departmentId": entityId },
        { $push: { "departmentList.$.programs": program } },
        { new: true }
      );
      let result = await Document.findOne(
        { "departmentList.departmentId": entityId },
      ...
      .on("end", async () => {
        ...
        inserter.map(async (prg) => {
          ...
          prg["admissionRequirement"] = [
            {
              ...
              content:
                prg["admissionRequirementOlevel"].toString() == ""
                  ? null
                  : prg["admissionRequirementOlevel"].toString(),
            },
            {
              ...
              content:
                prg["admissionRequirementUtme"].toString() == ""
                  ? null
                  : prg["admissionRequirementUtme"].toString(),
            },
            {
              ...
              content:
                prg["admissionRequirementDe"].toString() == ""
                  ? null
                  : prg["admissionRequirementDe"].toString(),
            },
            {
              ...
              content:
                prg["admissionRequirementPg"].toString() == ""
                  ? null
                  : prg["admissionRequirementPg"].toString(),
            },
          ];
          prg["graduationRequirements"] = [
            prg["graduationRequirements"].toString(),
          ];
          let prgName = prg.programName
            .toString()
            .split(" ")
            .slice(1)
            .join(" ");
          ...
          if (activity == "center" || activity == "unit") {
            ...
            await Document.findOneAndUpdate(
              { "departmentList.departmentId": entityId },
              { $push: { "departmentList.$.programs": dt } },
              { new: true }
            );
          }
          console.log(dt);
...
exports.addProgramBrochure = async (req, res, next) => {
  ...
  try {
    if (activity == "faculty")
      ...
      await broc.addBrochure(Unit, activity, programId, departmentId, req, res);
    else {
      res.json({ success: false, message: "Wrong parameters" });
    }
  } catch (error) {
...
exports.editHod = async (req, res, next) => {
  ...
  const hodEdit = async (Document) => {
    ...
    return await Document.findOne(
      { "departmentList.departmentId": departmentId },
      { _id: 0, "departmentList.staffList.password": 0 }
    );
  };
  ...
  res.json({
    success: true,
    message: `HOD from faculty with the ID ${departmentId} has been edited`,
    result,
  });
};
...
exports.editDepartmentProgram = async (req, res, next) => {
      ...
      await Document.findOneAndUpdate(
        { "programs.programId": programId },
        {
          $set: {
            ...
            "programs.$[e2].graduationRequirement":
              program.graduationRequirement,
            "programs.$[e2].careerProspect": program.careerProspect,
          },
        },
        {
          arrayFilters: [{ "e2.programId": programId }],
        }
      );
      ...
      await Document.findOneAndUpdate(
        { "departmentList.programs.programId": programId },
        {
          $set: {
            ...
            "departmentList.$[e1].programs.$[e2].admissionRequirement":
              program.admissionRequirement,
            "departmentList.$[e1].programs.$[e2].graduationRequirement":
              program.graduationRequirement,
            "departmentList.$[e1].programs.$[e2].careerProspect":
              program.careerProspect,
          },
        },
        {
          arrayFilters: [
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Program from department with the ID ${departmentId} has been edited`,
      result,
    });
  } catch (error) {
    console.log({ success: false, error });
...
exports.editDepartmentStaffs = async (req, res, next) => {
  ...
  const staffEdit = async (Document) => {
    if (activity == "center" || activity == "unit") {
      await Document.findOneAndUpdate(
        ...
        {
          arrayFilters: [{ "e2.staffId": staffId }],
        }
      );
      return await Document.findOne(
        { "staffList.staffId": staffId },
  ...
  res.json({
    success: true,
    message: `Staff from department with the ID ${departmentId} has been edited`,
    result,
  });
};
...
exports.removeHod = async (req, res, next) => {
  ...
  const hodRemove = async (Document) => {
    const resultImage = await Document.findOne(
      { "departmentList.departmentId": departmentId },
      { _id: 0, departmentList: 1 }
    );
  ...
  res.json({
    success: true,
    message: `HOD from department with the ID ${departmentId} has been removed`,
    result,
  });
};
...
exports.removeDepartmentProgram = async (req, res, next) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Program with the ID ${programId} has been removed`,
      result,
    });
  } catch (error) {
    console.log({ success: false, error });
  ...
  const removeProgram = async (Document) => {
    if (activity == "center" || activity == "unit") {
      ...
      await Document.findOneAndUpdate(
        ...
        {
          arrayFilters: [{ "e1.departmentId": departmentId }],
        }
      );
      return await Document.findOne(
        { "departmentList.departmentId": departmentId },
...
exports.removeDepartmentStaff = async (req, res, next) => {
  ...
  res.json({
    success: true,
    message: `Staff with the ID ${staffId} has been removed`,
    result,
  });
};
...
exports.removeDepartment = async (req, res, next) => {
  ...
  try {
    if (activity == "faculty")
      await departmentDelete(Faculty, departmentId, facultyId, req, res);
    else if (activity == "college")
      await departmentDelete(College, departmentId, facultyId, req, res);
    else if (activity == "school")
      await departmentDelete(School, departmentId, facultyId, req, res);
    else {
      res.json({ success: false, message: "Wrong parameters" });
    }
  } catch (error) {
...
exports.createAbout = async (req, res, next) => {
  singleUpload(req, res, async function (err) {
    ...
    if (err instanceof multer.MulterError) {
      ...
      if (
        req.body.activity != "chancellor" &&
        req.body.activity != "proChancellor"
      )
        req.body.evntId = randomstring.generate(8);
      ...
      res.json({
        success: true,
        message: "About created successfullty",
        result,
        newlyData: req.body,
      });
    }
...
exports.editAbout = async (req, res, next) => {
  ...
  if (activity == "council") {
    allEvents = await About.findOneAndUpdate(
      { "council.evntId": eventId },
      {
        $set: {
    ...
    allEvents = await About.findOneAndUpdate(
      { "principalOfficer.evntId": eventId },
      {
        $set: {
    ...
    allEvents = await About.updateOne(
      {},
      {
        $set: {
        ...
        if (activity == "chancellor") {
          ...
          cloudinary.v2.uploader.upload(
            ...
            async function (error, result) {
              ...
              allResults = await About.findOneAndUpdate(
                {},
                { $set: { "chancellor.image": result.secure_url } },
                { new: true }
              );
          ...
          cloudinary.v2.uploader.upload(
            ...
            async function (error, result) {
              ...
              allResults = await About.findOneAndUpdate(
                {},
                { $set: { "proChancellor.image": result.secure_url } },
                { new: true }
              );
          ...
          cloudinary.v2.uploader.upload(
            ...
            async function (error, result) {
              ...
              allResults = await About.findOneAndUpdate(
                { "principalOfficer.evntId": eventId },
                { $set: { "principalOfficer.$.image": result.secure_url } },
                { new: true }
              );
