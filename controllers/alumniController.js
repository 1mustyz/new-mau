...
exports.addAlumni = async (req,res,next) => {
    const {data,alumniType} = req.body
    ...
    if (alumni.length == 0){
      await Alumni.collection.insertOne({
        "dean" : [],
        "mainSlide": [],
        "bodySlide": [],
      })
    ...
    if(alumniType == "dean"){
      result = await Alumni.findOneAndUpdate({},{$set:{[alumniType]:data}},{new:true})
    }else{
  
      result = await Alumni.findOneAndUpdate({},{$push:{[alumniType]:data}},{new:true})
    }
  
    res.json({success: true, message: 'Alumni created successfullty', result, newlyEvent:data});
  }
  
  // edit event
exports.editAlumni = async (req,res,next) => {
    let allEvents
    const {eventId,evnt,eventName} = req.body;
    if(eventName == "mainSlide"){
  
      allEvents = await Alumni.findOneAndUpdate({"mainSlide.evntId": eventId},{$set: {
      ...
      allEvents = await Alumni.findOneAndUpdate({"bodySlide.evntId": eventId},{$set: {
    ...
    const result = await Alumni.findOne({},{_id: 0,[eventName]: 1})
  ...
  //   try{
  //     const singleMainEvent = await Alumni.aggregate([
  ...
  // delete or remove alumni event
exports.removeAlumniEvent = async (req,res,next) => {
    ...
    await Alumni.findOneAndUpdate({evntId:eventId},{$pull:{[eventName]:{evntId: eventId}}})
  ...
  // Add event pic
exports.addAlumniImage = async (req,res, next) => {
    const {eventName,eventId} = req.query
    let allResults
    try {
        ...
        try {
            if (eventName != "dean"){
                ...
                if(eventName == "mainSlide"){
                    ...
                    allResults = await Alumni.findOneAndUpdate({"mainSlide.evntId": eventId},{$set: {"mainSlide.$.image": result.secure_url}},{new:true})
                }else if(eventName == "bodySlide"){
                    allResults = await Alumni.findOneAndUpdate({"bodySlide.evntId": eventId},{$set: {"bodySlide.$.image": result.secure_url}},{new:true})
                }else{
                ...
                async function(error, result) {
                  ...
                  allResults = await Alumni.findOneAndUpdate({},{$set: {"dean.image": result.secure_url}},{new:true})
